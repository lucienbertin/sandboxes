...
const isSubInitialized = mqSubConnection.initialize().then((conn) => conn.connected);
...
export async function registerConsumer(consumer: RmqConsumer) {
    ...
    await mqSubConnection.channel.bindQueue(queue, consumer.exchange, consumer.routingKey);
    console.log(`queue is bound`);

    await mqSubConnection.channel.consume(queue, consumer.handler(mqSubConnection.channel));
