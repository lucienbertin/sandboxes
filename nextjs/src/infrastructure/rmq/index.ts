...
export async function publish(routingKey: string, message: unknown): Promise<void> {
    await isInitialized;

    await mqConnection.channel.publish(EXCHANGE_NAME, routingKey, Buffer.from(JSON.stringify(message)));
}