...
async fn rocket() -> _ {
    ...
    let queue = rmq_channel.queue_declare(
        "test",
        QueueDeclareOptions::default(),
        FieldTable::default(),
    ).await.expect("should create queue test");

    let mut consumer = rmq_channel.basic_consume(
        queue.name().as_str(),
        "test_consumer",
    ...
    async_global_executor::spawn(async move {
        while let Some(delivery) = consumer.next().await {
            ...
            println!("delivery key:{:?} | msg: {:?}", delivery.routing_key.as_str(), String::from_utf8(delivery.data.clone()));
            delivery
    ...
    rocket::build()
        ...
        .manage(ServerState { db_pool: db_pool, rmq_channel: rmq_channel })
}
