...
pub fn get_posts(
    ...
    let results = select_published_posts().map_err(|e| {
...
pub fn publish_post(
    ...
    match result {
        ...
        PublishPostResult::DoPublish(post_id) => {
            adapters::publish_post(post_id).map_err(|e| Error::from(e).into())
        }
    }?;
...
pub fn delete_post(
    ...
    use adapters::{delete_post, find_post};
...
pub fn patch_post(
    ...
    let result = post
        .map(|p| application::usecases::edit_post(&subject.email, &p, data.into_inner().into()));
